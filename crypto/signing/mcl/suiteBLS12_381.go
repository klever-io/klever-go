...
// For Klever the public keys for the validators are known during an epoch and also are not set on blocks
...
// CreatePointFromBytes -
func (s *SuiteBLS12) CreatePointFromBytes(b []byte) (crypto.Point, error) {
	point := s.G2.CreatePoint()
	return point, point.UnmarshalBinary(b)
}
