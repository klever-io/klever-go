...
func (stats *MemStatistics) String() string {
	return fmt.Sprintf("total:%s, percent:%d%%, go:%s, sys:%s, heap:%s, stack:%s",
		tools.ConvertBytes(stats.Total),
		stats.PercentUsed,
		tools.ConvertBytes(stats.UsedByGolang),
		tools.ConvertBytes(stats.UsedBySystem),
		tools.ConvertBytes(stats.HeapInUse),
		tools.ConvertBytes(stats.StackInUse),
	)
}
