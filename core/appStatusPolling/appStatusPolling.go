...
// makes that safe: it keeps wg.Go's Add from landing while Close is inside
// wg.Wait, which sync detects and answers with a process-killing
...
// this: it drains the background closers before closing the NetMessenger, Store
// and trie persisters those handlers read from.
//
...
// abandoned waiter goroutine exits on its own once the handler finally returns.
//
// Idempotent. Must not be called from inside a registered polling handler —
// that would deadlock on the join.
