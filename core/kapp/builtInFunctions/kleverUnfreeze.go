...
// NewKleverUnfreezeFunc returns the create asset built-in function component
func NewKleverUnfreezeFunc(
	funcGasCost uint64,
	marshaller vmcommon.Marshalizer,
	accountsCacher state.AccountsCacher,
	forkController core.ForkController,
	kappController kapp.KAppController,
) (*kleverUnfreeze, error) {
...
func (e *kleverUnfreeze) SetNewGasConfig(gasCost *vmcommon.GasCost) {
	if gasCost == nil {
		return
	}

	e.mutExecution.Lock()
	e.funcGasCost = gasCost.BuiltInCost.Unfreeze
	e.mutExecution.Unlock()
}

// ProcessBuiltinFunction resolves KDA transfer function calls
func (e *kleverUnfreeze) ProcessBuiltinFunction(vmInput *vmcommon.ContractCallInput) (*vmcommon.VMOutput, error) {
	e.mutExecution.RLock()
	defer e.mutExecution.RUnlock()

	contract, err := e.getUnfreezeContract(vmInput)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		log.Trace("Unfree error", "resultCode", resultCode, "err", err.Error())
		return nil, err
	}

	if resultCode != transaction.Transaction_Ok {
		err = fmt.Errorf("BucketUnfreeze error: %s", resultCode.String())
		log.Trace("Unfreeze error", "resultCode", resultCode, "err", err.Error())
		return nil, err
	}
...
// getUnfreezeContract convert the arguments to an UnfreezeContract
func (e *kleverUnfreeze) getUnfreezeContract(vmInput *vmcommon.ContractCallInput) (*transaction.UnfreezeContract, error) {
	if len(vmInput.Arguments) < core.MinLenArgumentsUnfreeze {
...
func (e *kleverUnfreeze) IsInterfaceNil() bool {
	return e == nil
}
