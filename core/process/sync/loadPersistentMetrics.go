...
func loadMetricsFromDb(store retriever.StorageService, uint64ByteSliceConverter typeConverters.Uint64ByteSliceConverter, marshalizer marshal.Marshalizer, nonce uint64,
) (map[string]uint64, map[string]string) {
	nonceBytes := uint64ByteSliceConverter.ToByteSlice(nonce)
	storer := store.GetStorer(retriever.StatusMetricsUnit)
...
func prepareMetricMaps(metricsMap map[string]interface{}) (map[string]uint64, map[string]string) {
	...
	uint64Map[core.MetricSlotAtEpochStart] = persister.GetUint64(metricsMap[core.MetricSlotAtEpochStart])
