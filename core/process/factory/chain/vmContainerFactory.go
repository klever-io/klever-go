...
import (
	...
	wasmvm10 "github.com/klever-io/klever-go/kvm/vmhost"
	wasmVMHost10 "github.com/klever-io/klever-go/kvm/vmhost/hostCore"
...
func (vmf *vmContainerFactory) validateWasmVMVersions() error {
	if len(vmf.wasmVMVersions) == 0 {
		return headerCheck.ErrEmptyVersionsByEpochsList
	}
	...
	for idx, ver := range vmf.wasmVMVersions {
		if idx == 0 && ver.StartEpoch != 0 {
			return fmt.Errorf("%w first version should start on epoch 0",
				headerCheck.ErrInvalidVersionOnEpochValues)
		}
...
func (vmf *vmContainerFactory) Create() (process.VirtualMachinesContainer, error) {
	...
	err = container.Add(common.WasmVirtualMachine, currentVM)
	if err != nil {
		vmf.wasmVMChangeLocker.Unlock()
		return nil, err
	}
...
func (vmf *vmContainerFactory) ensureCorrectWasmVMVersion(epoch uint32) {
	...
	currentWasmVM, err := vmf.container.Get(common.WasmVirtualMachine)
	if err != nil {
	...
	err = vmf.container.Replace(common.WasmVirtualMachine, newWasmVM)
	if err != nil {
		logVMContainerFactory.Error("cannot replace Wasm VM", "epoch", epoch, "error", err)
		return
	}
...
func (vmf *vmContainerFactory) createInProcessWasmVMV10() (vmcommon.VMExecutionHandler, error) {
	protectedKeys := [][]byte{
	...
	return wasmVMHost10.NewVMHost(vmf.blockChainHook, hostParameters)
}
