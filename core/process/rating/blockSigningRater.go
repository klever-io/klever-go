...
type BlockSigningRater struct {
	...
	maxRating             uint32
	minRating             uint32
	ratingsStepHandler    process.RatingsStepHandler
...
func NewBlockSigningRater(ratingsData process.RatingsInfoHandler) (*BlockSigningRater, error) {
	...
	return &BlockSigningRater{
		...
		ratingsStepHandler:    ratingsData.ChainRatingsStepHandler(),
...
func verifyRatingsData(ratingsData process.RatingsInfoHandler) error {
	...
	if ratingsData.ChainRatingsStepHandler().ConsecutiveMissedBlocksPenalty() < 1 {
		return fmt.Errorf("%w: chain consecutiveMissedBlocksPenalty: %v",
			process.ErrConsecutiveMissedBlocksPenaltyLowerThanOne,
			ratingsData.ChainRatingsStepHandler().ConsecutiveMissedBlocksPenalty())
	}
	if ratingsData.ChainRatingsStepHandler().ProposerDecreaseRatingStep() > -1 || ratingsData.ChainRatingsStepHandler().ValidatorDecreaseRatingStep() > -1 {
		return fmt.Errorf("%w: chain decrease steps - proposer: %v, validator: %v",
			process.ErrDecreaseRatingsStepMoreThanMinusOne,
			ratingsData.ChainRatingsStepHandler().ProposerDecreaseRatingStep(),
			ratingsData.ChainRatingsStepHandler().ValidatorDecreaseRatingStep())
	}
	return nil
}
...
func (bsr *BlockSigningRater) ComputeIncreaseProposer(currentRating uint32) uint32 {
	log.Trace("ComputeIncreaseProposer", "currentRating", currentRating)

	ratingStep := bsr.ratingsStepHandler.ProposerIncreaseRatingStep()
...
func (bsr *BlockSigningRater) RevertIncreaseValidator(currentRating uint32, nrReverts uint32) uint32 {
	log.Trace("RevertIncreaseValidator", "currentRating", currentRating, "nrReverts", nrReverts)

	ratingStep := bsr.ratingsStepHandler.ValidatorIncreaseRatingStep()
...
func (bsr *BlockSigningRater) ComputeDecreaseProposer(currentRating uint32, consecutiveMisses uint32) uint32 {
	log.Trace("ComputeDecreaseProposer", "currentRating", currentRating, "consecutiveMisses", consecutiveMisses)

	proposerDecreaseRatingStep := bsr.ratingsStepHandler.ProposerDecreaseRatingStep()
	consecutiveBlocksPenalty := bsr.ratingsStepHandler.ConsecutiveMissedBlocksPenalty()
...
func (bsr *BlockSigningRater) ComputeIncreaseValidator(currentRating uint32) uint32 {
	log.Trace("ComputeIncreaseValidator", "currentRating", currentRating)
...
func (bsr *BlockSigningRater) ComputeDecreaseValidator(currentRating uint32) uint32 {
	log.Trace("ComputeDecreaseValidator", "currentRating", currentRating)

	ratingStep := bsr.ratingsStepHandler.ValidatorDecreaseRatingStep()
	return bsr.computeRating(ratingStep, currentRating)
}
