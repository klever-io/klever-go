...
func (pph *p2pPeerHonesty) ChangeScore(pk string, topic string, units int) {
	...
	if change < 0 {
		log.Trace("p2pPeerHonesty.ChangeScore decrease",
			"pk", tools.GetTrimmedPk(hex.EncodeToString([]byte(pk))),
...
func (pph *p2pPeerHonesty) checkBlacklistNoLock(ps *peerScore) {
	...
	err := pph.blackListedPkCache.Upsert(ps.pk, tools.PublicKeyBlacklistDuration)
	if err != nil {
