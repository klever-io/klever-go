...
func TestP2PAntiflood_CanNotIncrementFromConnectedPeerShouldError(t *testing.T) {
	...
	assert.True(t, errors.Is(err, common.ErrSystemBusy))
}
...
func TestP2pAntiflood_IsOriginatorEligibleForTopic(t *testing.T) {
	...
	err = afm.IsOriginatorElectedForTopic(core.PeerID(validatorPID), "topicForAll1")
	assert.Nil(t, err)
	err = afm.IsOriginatorElectedForTopic(core.PeerID(validatorPID), "topic")
	assert.Nil(t, err)
}
