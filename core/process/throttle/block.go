...
// Add adds the new size for last block which has been sent in the given slot
func (bst *blockSizeThrottle) Add(slot uint64, size uint32) {
	bst.mutThrottler.Lock()
	bst.statistics = append(
		bst.statistics,
		&blockInfo{slot: slot, size: size, currentMaxSize: bst.currentMaxSize},
	)
...
// Succeed sets the state of the last block which has been sent in the given slot
func (bst *blockSizeThrottle) Succeed(slot uint64) {
	bst.mutThrottler.Lock()
	for i := len(bst.statistics) - 1; i >= 0; i-- {
		if bst.statistics[i].slot == slot {
...
func (bst *blockSizeThrottle) getMaxSizeWhenSucceed(lastActionMaxSize uint32) uint32 {
	...
	increasedMaxSize := tools.MaxUint32(1, uint32(float32(maxSizeUsedWithoutSucceed-lastActionMaxSize)*jumpAboveFactor))
...
func (bst *blockSizeThrottle) getMaxSizeWhenNotSucceed(lastActionMaxSize uint32) uint32 {
	...
	decreasedMaxSize := tools.MaxUint32(1, uint32(float32(lastActionMaxSize-maxSizeUsedWithSucceed)*jumpBelowFactor))
