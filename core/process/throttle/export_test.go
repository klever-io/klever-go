...
func (bst *blockSizeThrottle) SlotInLastSizeAdded() uint64 {
	bst.mutThrottler.RLock()
	defer bst.mutThrottler.RUnlock()

	return bst.statistics[len(bst.statistics)-1].slot
}
...
func (bst *blockSizeThrottle) SetSucceed(slot uint64, succeed bool) {
