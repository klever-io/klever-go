...
func NewSlotConsensusWrapper(rcns *slotConsensus) *SlotConsensus {
...
func (wrk *Worker) NewBlockProcessor(obj interface{}) {
	wrk.blockProcessor = &mock.BlockProcessorMock{
...
func (wrk *Worker) SlotManager() consensus.SlotManager {
	return wrk.slotManager
}
...
func (wrk *Worker) SetSlotManager(slotManager consensus.SlotManager) {
	wrk.slotManager = slotManager
}
...
func (cmv *consensusMessageValidator) IsMessageTypeLimitReached(pk []byte, slot int64, msgType consensus.MessageType) bool {
	return cmv.isMessageTypeLimitReached(pk, slot, msgType)
}

func (cmv *consensusMessageValidator) GetNumOfMessageTypeForPublicKey(pk []byte, slot int64, msgType consensus.MessageType) uint32 {
	cmv.mutPkConsensusMessages.RLock()
	defer cmv.mutPkConsensusMessages.RUnlock()

	key := fmt.Sprintf("%s_%d", string(pk), slot)
