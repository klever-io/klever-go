	...
	convertedContract := &data.TXContract{
		...
		Parameter: data.CreateValidatorContract{
			...
			Config: data.ValidatorConfig{
				BLSPublicKey:        hex.EncodeToString(createValidatorContract.GetConfig().GetBLSPublicKey()),
				RewardAddress:       cm.addressPubkeyConverter.Encode(createValidatorContract.GetConfig().GetRewardAddress()),
				CanDelegate:         createValidatorContract.GetConfig().GetCanDelegate(),
				Commission:          createValidatorContract.GetConfig().GetCommission(),
				MaxDelegationAmount: createValidatorContract.GetConfig().GetMaxDelegationAmount(),
				Logo:                createValidatorContract.GetConfig().GetLogo(),
				URIs:                cm.convertURIs(createValidatorContract.GetConfig().GetURIs()),
				Name:                createValidatorContract.GetConfig().GetName(),
			},
		},
	}
...
func (cm *commonProcessor) convertValidatorConfigContract(validatorConfigContract *transaction.ValidatorConfigContract) *data.TXContract {
	convertedContract := &data.TXContract{
		...
		Parameter: data.ValidatorConfigContract{
			Config: data.ValidatorConfig{
				BLSPublicKey:        hex.EncodeToString(validatorConfigContract.GetConfig().GetBLSPublicKey()),
				RewardAddress:       cm.addressPubkeyConverter.Encode(validatorConfigContract.GetConfig().GetRewardAddress()),
				CanDelegate:         validatorConfigContract.GetConfig().GetCanDelegate(),
				Commission:          validatorConfigContract.GetConfig().GetCommission(),
				MaxDelegationAmount: validatorConfigContract.GetConfig().GetMaxDelegationAmount(),
				Logo:                validatorConfigContract.GetConfig().GetLogo(),
				URIs:                cm.convertURIs(validatorConfigContract.GetConfig().GetURIs()),
				Name:                validatorConfigContract.GetConfig().GetName(),
			},
		},
	}
