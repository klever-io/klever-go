...
func (ir *interceptorResolver) parseConfig(config config.InterceptorResolverDebugConfig) error {
	...
	if config.IntervalAutoPrintInSeconds < minIntervalInSeconds {
		return fmt.Errorf("%w for IntervalAutoPrintInSeconds, minimum is %d", common.ErrInvalidValue, minIntervalInSeconds)
	}
	if config.NumRequestsThreshold < minThresholdRequests {
		return fmt.Errorf("%w for NumRequestsThreshold, minimum is %d", common.ErrInvalidValue, minThresholdRequests)
	}
	if config.NumResolveFailureThreshold < minThresholdResolve {
		return fmt.Errorf("%w for NumResolveFailureThreshold, minimum is %d", common.ErrInvalidValue, minThresholdResolve)
	}
	if config.DebugLineExpiration < minDebugLineExpiration {
		return fmt.Errorf("%w for DebugLineExpiration, minimum is %d", common.ErrInvalidValue, minDebugLineExpiration)
	}
