...
func InitLogHandler(args LogHandlerArgs) error {
	if args.Presenter == nil {
		return ErrNilConnectorPresenter
	}
	...
	go func() {
		for {
			...
			if err != nil {
				_, _ = args.Presenter.Write([]byte(fmt.Sprintf("connector websocket error, retrying in %v...", retryDuration)))
...
func startListeningOnWebSocket(presenter PresenterHandler, chanNodeIsStarting chan struct{}) {
	for {
		...
		if !isConnectionClosed {
			_, _ = presenter.Write([]byte(fmt.Sprintf("connector websocket error: %s", err.Error())))
		} else {
			_, _ = presenter.Write([]byte(fmt.Sprintf("connector websocket terminated: %s", err.Error())))
		}
