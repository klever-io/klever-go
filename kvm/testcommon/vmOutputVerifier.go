...
var log = logger.GetOrCreate("VMOutputVerifier")
...
func (v *VMOutputVerifier) WithTrace() *VMOutputVerifier {
	...
	for _, vm := range v.VmOutput.Logs {
		log.Trace("Data", "len", len(vm.Data))
		for i, x := range vm.Data {
			log.Trace("Data", i, string(x))
		}

		log.Trace("Topics", "len", len(vm.Topics))
		for i, x := range vm.Topics {
			log.Trace("Topics", i, string(x))
		}
	}
...
func (v *VMOutputVerifier) ContractInvalid() *VMOutputVerifier {
	return v.ReturnCode(vmcommon.VMContractInvalid)
}
...
func (v *VMOutputVerifier) FunctionNotFound() *VMOutputVerifier {
	return v.ReturnCode(vmcommon.VMFunctionNotFound)
}
...
// Transfers verifies if OutputTransfer(s) for the specified accounts are the same as the provided ones
...
func (v *VMOutputVerifier) Print() *VMOutputVerifier {
	...
	for address, account := range vmOutput.OutputAccounts {
		...
		for i, transfer := range account.OutputTransfers {
			...
			log.Trace("VMOutput", "| OutputTransfers["+fmt.Sprint(i)+"].Value", transfer.KDATransfers.KDAValue)
		}
