...
func (ei *ExprInterpreter) InterpretSubTree(obj orderedjson.OJsonObject) ([]byte, error) {
	if str, isStr := obj.(*orderedjson.OJsonString); isStr {
		return ei.InterpretString(str.Value)
	}

	if list, isList := obj.(*orderedjson.OJsonList); isList {
		var concat []byte
	...
	if mp, isMap := obj.(*orderedjson.OJsonMap); isMap {
		var concat []byte
...
func (ei *ExprInterpreter) interpretNumberWithSign(strRaw string, targetWidth int) ([]byte, error) {
	...
	if targetWidth == 0 {
		return twoscomplement.ToBytes(number), nil
	}

	return twoscomplement.ToBytesOfLength(number, targetWidth)
}
...
func (ei *ExprInterpreter) interpretUnsignedNumberFixedWidth(strRaw string, targetWidth int) ([]byte, error) {
	...
	return twoscomplement.CopyAlignRight(numberBytes, targetWidth), nil
}
...
func (ei *ExprInterpreter) interpretExplicitFloatingPointNumber(strRaw string) (bool, []byte, error) {
	...
	encodedLength := twoscomplement.CopyAlignRight(lengthBytes, 4)
