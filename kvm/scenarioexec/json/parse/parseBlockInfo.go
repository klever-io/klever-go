...
func (p *Parser) processBlockInfo(blockInfoRaw orderedjson.OJsonObject) (*scenjsonmodel.BlockInfo, error) {
	blockMap, isMap := blockInfoRaw.(*orderedjson.OJsonMap)
	if !isMap {
		return nil, errors.New("unmarshalled block info object is not a map")
	}
	blockInfo := &scenjsonmodel.BlockInfo{}
	var err error
