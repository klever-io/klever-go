...
func (p *Parser) ParseScenarioFile(jsonString []byte) (*scenjsonmodel.Scenario, error) {
	...
	scenario := &scenjsonmodel.Scenario{
		CheckGas:    true,
		TraceGas:    false,
		GasSchedule: scenjsonmodel.GasScheduleDefault,
	}
	...
	for _, kvp := range topMap.OrderedKV {
		...
		case "traceGas":
			traceGasOJ, isBool := kvp.Value.(*orderedjson.OJsonBool)
			if !isBool {
...
func (p *Parser) parseGasSchedule(value orderedjson.OJsonObject) (scenjsonmodel.GasSchedule, error) {
	gasScheduleStr, err := p.parseString(value)
	if err != nil {
		return scenjsonmodel.GasScheduleDummy, fmt.Errorf("gasSchedule type not a string: %w", err)
	}
...
func (p *Parser) processScenarioStepList(obj interface{}) ([]scenjsonmodel.Step, error) {
	listRaw, listOk := obj.(*orderedjson.OJsonList)
	if !listOk {
		return nil, errors.New("steps not a JSON list")
	}
	var stepList []scenjsonmodel.Step
...
func (p *Parser) ParseScenarioStep(jsonSnippet string) (scenjsonmodel.Step, error) {
	jobj, err := orderedjson.ParseOrderedJSON([]byte(jsonSnippet))
	if err != nil {
		return nil, err
	}
...
func (p *Parser) processScenarioStep(stepObj orderedjson.OJsonObject) (scenjsonmodel.Step, error) {
	stepMap, isStepMap := stepObj.(*orderedjson.OJsonMap)
	if !isStepMap {
	...
	case scenjsonmodel.StepNameExternalSteps:
		traceGasStatus := scenjsonmodel.Undefined
		step := &scenjsonmodel.ExternalStepsStep{TraceGas: traceGasStatus}
		for _, kvp := range stepMap.OrderedKV {
			switch kvp.Key {
			case "step":
			case "comment":
		...
		return step, nil
	case scenjsonmodel.StepNameSetState:
		step := &scenjsonmodel.SetStateStep{}
		for _, kvp := range stepMap.OrderedKV {
			switch kvp.Key {
			case "step":
			case "id":
		...
		return step, nil
	case scenjsonmodel.StepNameCheckState:
		step := &scenjsonmodel.CheckStateStep{}
		for _, kvp := range stepMap.OrderedKV {
			switch kvp.Key {
			case "step":
			case "id":
		...
		return step, nil
	case scenjsonmodel.StepNameDumpState:
		step := &scenjsonmodel.DumpStateStep{}
		for _, kvp := range stepMap.OrderedKV {
			switch kvp.Key {
			case "step":
			case "comment":
		...
		return step, nil
	case scenjsonmodel.StepNameScCall:
		return p.parseTxStep(scenjsonmodel.ScCall, stepMap)
	case scenjsonmodel.StepNameScDeploy:
		return p.parseTxStep(scenjsonmodel.ScDeploy, stepMap)
	case scenjsonmodel.StepNameScUpgrade:
		return p.parseTxStep(scenjsonmodel.ScUpgrade, stepMap)
	case scenjsonmodel.StepNameScQuery:
...
func (p *Parser) parseTxStep(txType scenjsonmodel.TransactionType, stepMap *orderedjson.OJsonMap) (*scenjsonmodel.TxStep, error) {
	step := &scenjsonmodel.TxStep{}
	var err error
	for _, kvp := range stepMap.OrderedKV {
		switch kvp.Key {
		case "step":
