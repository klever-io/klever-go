...
func (p *Parser) processTx(txType scenjsonmodel.TransactionType, blrRaw orderedjson.OJsonObject) (*scenjsonmodel.Transaction, error) {
	bltMap, isMap := blrRaw.(*orderedjson.OJsonMap)
	if !isMap {
		return nil, errors.New("unmarshalled transaction is not a map")
	}

	blt := scenjsonmodel.Transaction{
		Type:     txType,
		KLVValue: scenjsonmodel.JSONBigIntZero(),
		KDAValue: nil,
	}
	...
	for _, kvp := range bltMap.OrderedKV {
		...
		case "to":
			...
			if txType == scenjsonmodel.ScDeploy {
				if len(toStr) > 0 {
		...
		case "klvValue":
			if !txType.HasValue() {
				return nil, errors.New("`klvValue` not allowed in this context")
			}
			blt.KLVValue, err = p.processBigInt(kvp.Value, bigIntUnsignedBytes)
			if err != nil {
				return nil, fmt.Errorf("invalid transaction klvValue: %w", err)
			}
		case "kda":
			// backwards compatibility
			fallthrough
		case "kdaValue":
			if !txType.HasKDA() {
				return nil, errors.New("`kdaValue` not allowed in this context")
			}
			blt.KDAValue, err = p.processTxKDA(kvp.Value)
			if err != nil {
				return nil, fmt.Errorf("invalid transaction kdaValue: %w", err)
			}
		case "arguments":
			...
			if txType == scenjsonmodel.Transfer && len(blt.Arguments) > 0 {
		...
		case "contractCode":
			...
			if txType != scenjsonmodel.ScDeploy && txType != scenjsonmodel.ScUpgrade && len(blt.Code.Value) > 0 {
		...
		case "codeMetadata":
			...
			if txType != scenjsonmodel.ScDeploy && txType != scenjsonmodel.ScUpgrade && len(blt.CodeMetadata.Value) > 0 {
