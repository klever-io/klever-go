...
type JSONCheckBytes struct {
	Value       []byte
	IsStar      bool
	Original    orderedjson.OJsonObject
	Unspecified bool
}
...
func JSONCheckBytesUnspecified() JSONCheckBytes {
	return JSONCheckBytes{
		Value:       []byte{},
		IsStar:      false,
		Original:    &orderedjson.OJsonString{Value: ""},
		Unspecified: true,
	}
}
...
func JSONCheckBytesReconstructed(value []byte, originalString string) JSONCheckBytes {
	return JSONCheckBytes{
		Value:       value,
		IsStar:      false,
		Original:    &orderedjson.OJsonString{Value: originalString},
		Unspecified: false,
	}
}
...
func (jcbytes JSONCheckBytes) OriginalEmpty() bool {
	if str, isStr := jcbytes.Original.(*orderedjson.OJsonString); isStr {
