...
func (executorMock *ExecutorMock) CreateAndStoreInstanceMock(t testing.TB, host vmhost.VMHost, code []byte, codeHash []byte, codeMetadata []byte, ownerAddress []byte, balance int64, createAccount bool) *InstanceMock {
	...
	if createAccount {
		...
		executorMock.World.PutAccount(account)
	}
...
func (executorMock *ExecutorMock) NewInstanceWithOptions(
	...
	we, _ := wasmer2.CreateExecutor()

	return we.NewInstanceWithOptions(contractCode, options)
}
...
func (executorMock *ExecutorMock) NewInstanceFromCompiledCodeWithOptions(
	...
	return we.NewInstanceFromCompiledCodeWithOptions(compiledCode, options)
}
