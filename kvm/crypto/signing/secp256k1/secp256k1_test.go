...
func TestBitcoinSig_CanonicalSignatureHighOrderShouldError(t *testing.T) {
	t.Parallel()
	...
	verifier := NewSecp256k1()
	_, err := verifier.EncodeSecp256k1DERSignature(r, s)
	assert.Equal(t, signing.ErrNonCanonicalSignature, err)
}
...
func TestSignatureMalleability(t *testing.T) {
	...
	_, success1 := order.SetString(orderStr, 16) // convert group order
	assert.True(t, success1, "Failed to convert hex string to big int")

	s2 := new(big.Int)
	_, success2 := s2.SetString("4a691139ad57a3f0b906637673aa2f63d1f55cb1a69199d4009eea23ceaddc93", 16) // convert s part
	if !success2 {
