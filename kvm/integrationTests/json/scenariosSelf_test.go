...
func TestScenariosSelfTest(t *testing.T) {
	ScenariosTest(t).
		...
		Exclude("scenarios-self-test/builtin-func-kda-transfer.scen.json").
		Exclude("scenarios-self-test/kda-zero-balance-check-err.scen.json").
		Exclude("scenarios-self-test/kda-non-zero-balance-check-err.scen.json").
		Run().
		CheckNoError()
}
...
func TestSetAccountSCAddressErr1(t *testing.T) {
	ScenariosTest(t).
		...
		Run().
		RequireError(
			"\"setState\" step validation failed for account \"address:not-a-sc-address\": account has code but not a smart contract address: 6e6f742d612d73632d616464726573735f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f")
}
...
func TestScenariosCheckKDAErr1(t *testing.T) {
	ScenariosTest(t).
		Folder("scenarios-self-test/set-check").
		File("set-check-kda.err1.json").
		Run().
		RequireError(
...
func TestScenariosKdaZeroBalance(t *testing.T) {
	ScenariosTest(t).
		Folder("scenarios-self-test").
		File("kda-zero-balance-check-err.scen.json").
		Run().
		RequireError(
...
func TestScenariosKdaNonZeroBalance(t *testing.T) {
	ScenariosTest(t).
		Folder("scenarios-self-test").
		File("kda-non-zero-balance-check-err.scen.json").
		Run().
		RequireError(
