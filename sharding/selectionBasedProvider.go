...
// elected list. A comparison between a real reslicing and this can be found in common_test.go
...
func (sbp *SelectionBasedProvider) Get(randomness []byte, numValidators int64, expandedElectedList []uint32) ([]uint32, error) {
	if len(randomness) == 0 {
		return nil, ErrNilRandomness
	}
	if numValidators > int64(len(expandedElectedList)) {
		return nil, ErrInvalidSampleSize
	}

	validators := make([]uint32, 0, numValidators)
	var index uint64
	lenExpandedList := int64(len(expandedElectedList))
