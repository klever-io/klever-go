...
func (txMap *txListBySenderMap) addTx(tx *WrappedTransaction) (bool, [][]byte) {
	...
	return listForSender.AddTx(tx, txMap.txFeeHelper)
}
...
// removeTx removes a transaction from the map
func (txMap *txListBySenderMap) removeTxLowerNonce(sender []byte, nonce uint64) [][]byte {
	removed := make([][]byte, 0)
	senderKey := string(sender)
	listForSender, ok := txMap.getListForSender(senderKey)
	if !ok {
		return removed
	}

	var lastRemovedNonce uint64
	for element := listForSender.items.Front(); element != nil; {
		tx, ok := element.Value.(*WrappedTransaction)
		if !ok {
			continue
		}
		if tx.Tx.GetNonce() > nonce {
			break
		}
...
func (txMap *txListBySenderMap) PendingFor(accountKey []byte) (uint64, uint64) {
	sender := string(accountKey)
	listForSender, ok := txMap.getListForSender(sender)
	if !ok {
		return 0, 0
	...
	for element := listForSender.items.Front(); element != nil; element = element.Next() {
		...
		if pendingNonce == 0 || tx.Tx.GetNonce() == pendingNonce+1 {
			pendingNonce = tx.Tx.GetNonce()
			continue
		}
	...
	return pendingNonce, listForSender.countTx()
}
