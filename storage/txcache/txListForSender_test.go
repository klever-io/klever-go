...
func TestListForSender_AddTx_Sorts(t *testing.T) {
	...
	list.AddTx(createTxWithDelay([]byte("a"), ".", 1, 1))
	list.AddTx(createTxWithDelay([]byte("c"), ".", 3, 1))
	list.AddTx(createTxWithDelay([]byte("d"), ".", 4, 1))
	list.AddTx(createTxWithDelay([]byte("b"), ".", 2, 1))
...
func TestListForSender_AddTx_GivesPriorityToHigherFees(t *testing.T) {
	...
	list.AddTx(createTxWithParams([]byte("a"), ".", 1, 128, 42))
	list.AddTx(createTxWithParams([]byte("b"), ".", 3, 128, 100))
	list.AddTx(createTxWithParams([]byte("c"), ".", 3, 128, 99))
	list.AddTx(createTxWithParams([]byte("d"), ".", 2, 128, 42))
	list.AddTx(createTxWithParams([]byte("e"), ".", 3, 128, 101))
...
func TestListForSender_AddTx_SortsCorrectlyWhenSameNonceSamePrice(t *testing.T) {
	...
	list.AddTx(createTxWithParams([]byte("a"), ".", 1, 128, 42))
	list.AddTx(createTxWithParams([]byte("b"), ".", 3, 128, 100))
	list.AddTx(createTxWithParams([]byte("c"), ".", 3, 128, 100))
	list.AddTx(createTxWithParams([]byte("d"), ".", 3, 128, 98))
	list.AddTx(createTxWithParams([]byte("e"), ".", 3, 128, 101))
	list.AddTx(createTxWithParams([]byte("f"), ".", 2, 128, 42))
	list.AddTx(createTxWithParams([]byte("g"), ".", 3, 128, 99))
...
func TestListForSender_AddTx_IgnoresDuplicates(t *testing.T) {
	...
	added, _ := list.AddTx(createTx([]byte("tx1"), ".", 1, 1))
	require.True(t, added)
	added, _ = list.AddTx(createTx([]byte("tx2"), ".", 2, 1))
	require.True(t, added)
	added, _ = list.AddTx(createTx([]byte("tx3"), ".", 3, 1))
	require.True(t, added)
...
func TestListForSender_AddTx_AppliesSizeConstraintsForNumTransactions(t *testing.T) {
	...
	list.AddTx(createTxWithDelay([]byte("tx1"), ".", 1, 1))
	list.AddTx(createTxWithDelay([]byte("tx5"), ".", 5, 1))
	list.AddTx(createTxWithDelay([]byte("tx4"), ".", 4, 1))
	list.AddTx(createTxWithDelay([]byte("tx2"), ".", 2, 1))
	require.Equal(t, []string{}, list.getTxHashesAsStrings())

	_, evicted := list.AddTx(createTxWithDelay([]byte("tx3"), ".", 3, 1))
	...
	_, evicted = list.AddTx(createTxWithParams([]byte("tx2++"), ".", 2, 128, 42))
	require.Equal(t, []string{"tx2++", "tx3"}, list.getTxHashesAsStrings())
	require.Equal(t, []string{}, hashesAsStrings(evicted))

	// Though Undesirably to some extent, "tx3++"" is added, then evicted
	_, evicted = list.AddTx(createTxWithParamsAndDelay([]byte("tx3++"), ".", 3, 128, 42))
...
func TestListForSender_AddTx_AppliesSizeConstraintsForNumBytes(t *testing.T) {
	...
	list.AddTx(createTxWithParamsAndDelay([]byte("tx1"), ".", 1, 128, 42))
	list.AddTx(createTxWithParamsAndDelay([]byte("tx2"), ".", 2, 512, 42))
	list.AddTx(createTxWithParamsAndDelay([]byte("tx3"), ".", 3, 256, 42))

	_, evicted := list.AddTx(createTxWithParamsAndDelay([]byte("tx5"), ".", 4, 256, 42))
	require.Equal(t, []string{}, list.getTxHashesAsStrings())
	require.Equal(t, []string{"tx1", "tx2", "tx3", "tx5"}, hashesAsStrings(evicted))

	_, evicted = list.AddTx(createTxWithParamsAndDelay([]byte("tx5--"), ".", 4, 128, 42))
	require.Equal(t, []string{"tx5--"}, list.getTxHashesAsStrings())
	require.Equal(t, []string{}, hashesAsStrings(evicted))

	_, evicted = list.AddTx(createTxWithParamsAndDelay([]byte("tx4"), ".", 4, 128, 42))
	...
	_, evicted = list.AddTx(createTxWithParams([]byte("tx3++"), ".", 3, 256, 100))
...
func TestListForSender_findTx_CoverNonceComparisonOptimization(t *testing.T) {
	list := newUnconstrainedListToTest()
	list.AddTx(createTx([]byte("A"), ".", 42, 1))
...
func TestListForSender_SelectBatchTo(t *testing.T) {
	...
	for index := 0; index < 100; index++ {
		list.AddTx(createTx([]byte{byte(index)}, ".", uint64(index), 1))
	}
...
func TestListForSender_SelectBatchTo_WhenInitialGap(t *testing.T) {
	...
	for index := 10; index < 20; index++ {
		list.AddTx(createTx([]byte{byte(index)}, ".", uint64(index), 1_500_000))
	}
...
func TestListForSender_SelectBatchTo_WhenGracePeriodWithGapResolve(t *testing.T) {
	...
	// Now resolve the gap
	list.AddTx(createTx([]byte("resolving-tx"), ".", 1, 1_500_000))
...
func TestListForSender_hasInitialGap(t *testing.T) {
	...
	// One gap
	list.AddTx(createTx([]byte("tx-43"), ".", 43, 1_500_000))
	require.True(t, list.hasInitialGap())
	// Resolve gap
	list.AddTx(createTx([]byte("tx-42"), ".", 42, 1_500_000))
	require.False(t, list.hasInitialGap())
}
...
func TestListForSender_getTxHashes(t *testing.T) {
	...
	list.AddTx(createTx([]byte("A"), ".", 1, 1))
	require.Len(t, list.getTxHashes(), 1)

	list.AddTx(createTx([]byte("B"), ".", 2, 1))
	list.AddTx(createTx([]byte("C"), ".", 3, 1))
...
func TestListForSender_DetectRaceConditions(t *testing.T) {
	...
	go func() {
		list.AddTx(createTx([]byte("test"), ".", 42, 1))
	}()
}
