...
func TestEviction_EvictSendersWhileTooManyTxs(t *testing.T) {
	...
	for index := 0; index < 200; index++ {
		...
		cache.AddTx(createTx([]byte{byte(index)}, sender, uint64(1), int64(1)))
	}
...
func TestEviction_EvictSendersWhileTooManyBytes(t *testing.T) {
	...
	for index := 0; index < 200; index++ {
		sender := string(createFakeSenderAddress(index))
		cache.AddTx(createTxWithParams([]byte{byte(index)}, sender, uint64(1), uint64(numBytesPerTx), 10000))
	}
...
func TestEviction_DoEvictionDoneInPassTwo_BecauseOfCount(t *testing.T) {
	...
	cache.AddTx(createTxWithParams([]byte("hash-alice"), "alice", uint64(1), 1000, 100000))
	cache.AddTx(createTxWithParams([]byte("hash-bob"), "bob", uint64(1), 1000, 200000))
	cache.AddTx(createTxWithParams([]byte("hash-carol"), "carol", uint64(1), 1000, 300000))
...
func TestEviction_DoEvictionDoneInPassTwo_BecauseOfSize(t *testing.T) {
	...
	cache.AddTx(createTxWithParams([]byte("hash-alice"), "alice", uint64(1), 128, 1_500_000))
	cache.AddTx(createTxWithParams([]byte("hash-bob"), "bob", uint64(1), 128, 1_500_000))
	cache.AddTx(createTxWithParams([]byte("hash-dave1"), "dave", uint64(3), 128, 1_500_000))
	cache.AddTx(createTxWithParams([]byte("hash-dave2"), "dave", uint64(1), 128, 1_500_000))
	cache.AddTx(createTxWithParams([]byte("hash-dave3"), "dave", uint64(2), 128, 1_500_000))
	cache.AddTx(createTxWithParams([]byte("hash-chris"), "chris", uint64(1), 128, 1_500_000))
	cache.AddTx(createTxWithParams([]byte("hash-richard"), "richard", uint64(1), 128, 3_000_000_000))
	cache.AddTx(createTxWithParams([]byte("hash-carol"), "carol", uint64(1), 128, 10_000_000_000))
	cache.AddTx(createTxWithParams([]byte("hash-eve"), "eve", uint64(1), 128, 5_000_000_000))
	...
	require.Equal(t, uint32(21), scoreAlice)
	require.Equal(t, uint32(21), scoreBob)
	require.Equal(t, uint32(16), scoreDave)
	require.Equal(t, uint32(92), scoreCarol)
	require.Equal(t, uint32(89), scoreEve)
	require.Equal(t, uint32(21), scoreChris)
	require.Equal(t, uint32(85), scoreRichard)
...
func TestEviction_doEvictionDoesNothingWhenAlreadyInProgress(t *testing.T) {
	...
	cache.AddTx(createTx([]byte("hash-alice"), "alice", uint64(1), int64(1)))
...
func TestEviction_evictSendersWhile_ShouldContinueBreak(t *testing.T) {
	...
	cache.AddTx(createTx([]byte("hash-alice"), "alice", uint64(1), int64(1)))
	cache.AddTx(createTx([]byte("hash-bob"), "bob", uint64(1), int64(1)))
...
func Test_EvictSendersAndTheirTxs_Concurrently(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func() {
			cache.AddTx(createTx([]byte("alice-x"), "alice", 42, 123))
			cache.AddTx(createTx([]byte("alice-y"), "alice", 43, 123))
			cache.AddTx(createTx([]byte("bob-x"), "bob", 42, 123))
			cache.AddTx(createTx([]byte("bob-y"), "bob", 43, 123))
