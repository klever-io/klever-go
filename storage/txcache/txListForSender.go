...
import (
	...
	"github.com/klever-io/klever-go/storage/txcache/maps"
...
func (listForSender *txListForSender) applySizeConstraints() [][]byte {
	...
	// remove old prior shrink
	for element := listForSender.items.Front(); element != nil; {
		value, ok := element.Value.(*WrappedTransaction)
		if !ok {
			continue
		}
		...
		if value.ExpireOn < time.Now().Unix() {
			listForSender.items.Remove(toRemove)
			listForSender.onRemovedListElement(toRemove)
			// Keep track of removed transactions
			evictedTxHashes = append(evictedTxHashes, value.TxHash)
		}
	}

	log.Debug("applySizeConstraints.Expired", "evicted", len(evictedTxHashes))
...
func (listForSender *txListForSender) getScoreParams() senderScoreParams {
	fees := listForSender.totalFees.GetUint64()
	size := listForSender.totalBytes.GetUint64()
	count := listForSender.countTx()

	return senderScoreParams{count: count, size: size, fees: fees}
}
...
func (listForSender *txListForSender) findInsertionPlace(incomingTx *WrappedTransaction) (*list.Element, error) {
	...
	incomingTxFee := incomingTx.Tx.GetTotalFees()

	for element := listForSender.items.Back(); element != nil; element = element.Prev() {
		currentTx := element.Value.(*WrappedTransaction)
		currentTxNonce := currentTx.Tx.GetNonce()
		currentTxFee := currentTx.Tx.GetTotalFees()
		...
		if currentTxNonce == incomingNonce && currentTxFee > incomingTxFee {
			// The incoming transaction will be placed right after the existing one, which has same nonce but higher price.
			// If the nonces are the same, but the incoming fee is higher or equal, the search loop continues.
			return element, nil
		}
...
func (listForSender *txListForSender) selectBatchTo(isFirstBatch bool, destination []*WrappedTransaction, batchSize int, bandwidth uint64) batchSelectionJournal {
	...
	for ; ; copied, copiedBandwidth = copied+1, copiedBandwidth+lastTxGasLimit {
		// if we reached the end of the list, or we copied the requested batch size, or we filled the available space, or we reached the bandwidth limit
		// then we stop copying
