...
func initPruningStorer(
	...
	pdb := &PruningStorer{
		...
		cacher:                cache,
		epochPrepareHdr:       &block.Block{Header: &block.BlockHeader{Epoch: epochForDefaultEpochPrepareHdr}},
		bloomFilter:           nil,
...
func (ps *PruningStorer) saveHeaderForEpochStartPrepare(header data.HeaderHandler) error {
	...
	var ok bool
	ps.epochPrepareHdr, ok = header.(*block.Block)
	if !ok {
...
func (ps *PruningStorer) changeEpoch(header data.HeaderHandler) error {
	...
	filePath := ps.pathManager.PathForEpoch(epoch, ps.identifier)
...
func (ps *PruningStorer) extendSavedEpochsIfNeeded(header data.HeaderHandler) bool {
	epoch := header.GetEpoch()
	metaBlock, mbOk := header.(*block.Block)
	if !mbOk {
	...
	shouldExtend := metaBlock.Header.Epoch != epochForDefaultEpochPrepareHdr
	if !shouldExtend {
		return false
	}
...
func createPersisterPathForEpoch(args *StorerArgs, epoch uint32, shard string) string {
	filePath := args.PathManager.PathForEpoch(epoch, args.Identifier)
	if len(shard) > 0 {
		filePath += shard
	}
...
func computeOldestEpoch(metaBlock *block.Block) uint32 {
	oldestEpoch := metaBlock.Header.Epoch
