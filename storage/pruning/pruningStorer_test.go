...
func getDefaultArgsSerialDB() *pruning.StorerArgs {
	...
	pathManager := &mock.PathManagerStub{PathForEpochCalled: func(epoch uint32, identifier string) string {
		return fmt.Sprintf("TestOnly-Epoch_%d/%s", epoch, identifier)
	}}
	return &pruning.StorerArgs{
		PruningEnabled:        true,
		Identifier:            "id",
...
func TestNewPruningStorer_InvalidNumberOfActivePersistersShouldErr(t *testing.T) {
	...
	assert.Nil(t, ps)
	assert.Equal(t, common.ErrInvalidNumberOfPersisters, err)
}
...
func TestNewPruningStorer_NilPathManagerShouldErr(t *testing.T) {
	...
	assert.Nil(t, ps)
	assert.Equal(t, common.ErrNilPathManager, err)
}
...
func TestNewPruningStorer_NilPersisterFactoryShouldErr(t *testing.T) {
	...
	assert.Nil(t, ps)
	assert.Equal(t, common.ErrNilPersisterFactory, err)
}
