...
import (
	...
	"github.com/klever-io/klever-go/storage/bloom"
	"github.com/klever-io/klever-go/storage/fifocache"
...
func NewStorageUnitFromConfWithSleep(cacheConf CacheConfig, dbConf DBConfig, bloomFilterConf BloomConfig, sleepBetween time.Duration) (*Unit, error) {
	...
	db, err = NewDBWithSleep(argDB, sleepBetween)
	if err != nil {
		return nil, err
	}
...
func NewDB(argDB ArgDB) (storage.Persister, error) {
	return NewDBWithSleep(argDB, core.SleepTimeBetweenCreateDBRetries)
}

func NewDBWithSleep(argDB ArgDB, sleepBetween time.Duration) (storage.Persister, error) {
	var db storage.Persister
	var err error
