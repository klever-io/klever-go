...
type blockResponseData struct {
	Block dataBlock.Block `json:"block"`
}
...
func TestGetBlockByNonce_ShouldWork(t *testing.T) {
	...
	facade := mock.Facade{
		GetBlockByNonceCalled: func(_ uint64, _ bool) (*api.Block, error) {
			return &api.Block{Block: &expectedBlock}, nil
		},
	}
	...
	assert.Equal(t, expectedBlock.Clone(), response.Data.Block.Clone())
}
