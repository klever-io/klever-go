...
- `logs` filters by every address a log entry is relevant to: the entry's own address (the invoked contract, or the *sender* for a deployment — see `contractId` below) plus each individual event's own address. A subscriber to a nested cross-contract call's inner contract, or to a just-deployed contract's own address, is reached the same way a subscriber to the outer/deployer address is.
...
| `address` | The relevant address (populated for address-specific events, empty for global). For `logs`, this is whichever address your subscription matched on — the entry's own address or one of its events' addresses (see Subscription Types above) — not necessarily `data.address`. |
...
`address`/`caller` are bech32-encoded; `topics`/`data` are hex-encoded byte strings, matching the shape already used for the Elasticsearch `logs` index. `caller`, `status` and `resultCode` mirror the producing transaction's sender/status/result code; they're omitted when the log's transaction hash isn't found in the block's transaction map. `status`/`resultCode` let a subscriber tell a log produced by a reverted/failed transaction apart from one whose effects actually committed — a transaction can still emit logs on its error path. `isSystemLog` marks an event generated by the node itself (e.g. an internal VM error) rather than by contract code, so a subscriber can filter node-internal noise out of a live feed; it's omitted (defaults to `false`) for ordinary contract-emitted events. `contractId` is the log-producing contract's index within its transaction's `Contracts` list (`kappContext.ContractID()`) — almost always `0`, since most transactions carry exactly one contract — not a per-contract identifier; it was previously always `0` on the Elasticsearch side before this field was wired up, regardless of index.

If the node operator has configured a mirror endpoint (`postConnectionURL`), delivery there is best-effort: a sustained slow or unresponsive mirror sheds events (dropped, not queued indefinitely) rather than applying backpressure to block processing.
