...
func Test_NewShardedTxPool(t *testing.T) {
	...
	require.Implements(t, (*retriever.ShardedDataCacherNotifier)(nil), pool)
}
...
func Test_NewShardedTxPool_WhenBadConfig(t *testing.T) {
	...
	require.Errorf(t, err, common.ErrCacheConfigInvalidSizeInBytes.Error())
	...
	require.Nil(t, pool)
	require.NotNil(t, err)
	require.Errorf(t, err, common.ErrCacheConfigInvalidSize.Error())
	...
	require.Nil(t, pool)
	require.NotNil(t, err)
	require.Errorf(t, err, common.ErrCacheConfigInvalidShards.Error())
}
...
func Test_NewShardedTxPool_ComputesCacheConfig(t *testing.T) {
	...
	require.Equal(t, true, pool.configPrototype.EvictionEnabled)
	require.Equal(t, 209715200, int(pool.configPrototype.NumBytesThreshold))
	require.Equal(t, 614400, int(pool.configPrototype.NumBytesPerSenderThreshold))
	require.Equal(t, 1000, int(pool.configPrototype.CountPerSenderThreshold))
	require.Equal(t, 100, int(pool.configPrototype.NumSendersToPreemptivelyEvict))
	require.Equal(t, 300000, int(pool.configPrototype.CountThreshold))
}
...
func Test_IsInterfaceNil(t *testing.T) {
	...
	makeNil := func() retriever.ShardedDataCacherNotifier {
		return nil
	}
...
func createTx(sender string, nonce uint64) *transaction.Transaction {
	return transaction.NewBaseTransaction([]byte(sender), nonce, [][]byte{[]byte("data")}, 0, 0)
}
...
func newTxPoolToTest() (retriever.ShardedDataCacherNotifier, error) {
	config := storageUnit.CacheConfig{
		Capacity:             100,
		SizePerSender:        10,
		SizeInBytes:          409600,
