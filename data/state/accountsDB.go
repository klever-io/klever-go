...
var numAccountsCheckpointsKey = []byte("state checkpoint")
...
func getNumAccountsCheckpoints(trieStorageManager data.StorageManager) uint32 {
	val, err := trieStorageManager.Database().Get(numAccountsCheckpointsKey)
	if err != nil {
		return 0
	}
...
func (adb *AccountsDB) ImportAccount(account AccountHandler) error {
	...
	if check.IfNil(account) {
		return fmt.Errorf("%w in accountsDB ImportAccount", common.ErrNilAccountHandler)
	}
...
func (adb *AccountsDB) SaveAccount(account AccountHandler) error {
	...
	if check.IfNil(account) {
		return fmt.Errorf("%w in accountsDB SaveAccount", common.ErrNilAccountHandler)
	}
...
func (adb *AccountsDB) saveCodeAndDataTrie(oldAcc, newAcc AccountHandler) error {
	...
	return adb.saveCode(newAcc, oldAcc)
}

func (adb *AccountsDB) saveCode(newAccount, oldAccount AccountHandler) error {
	newAcc, ok := newAccount.(UserAccountHandler)
	if !ok {
		return nil
	}
	oldAcc, _ := oldAccount.(UserAccountHandler)
...
func (adb *AccountsDB) saveDataTrie(accountHandler baseAccountHandler) error {
	if check.IfNil(accountHandler.DataTrieTracker()) {
		return common.ErrNilTrackableDataTrie
	}
	...
	entry, err := NewJournalEntryAccountDataTrieUpdates(oldValues, accountHandler)
	if err != nil {
		return err
	}
	adb.journalize(entry)
...
func (adb *AccountsDB) RemoveAccount(address []byte) error {
	...
	if check.IfNil(acnt) {
		return fmt.Errorf("%w in RemoveAccount for address %s", common.ErrAccNotFound, address)
	}
...
func (adb *AccountsDB) removeCode(baseAcc UserAccountHandler) error {
...
func (adb *AccountsDB) removeDataTrie(baseAcc baseAccountHandler) error {
	...
	entry, err := NewJournalEntryAccountDataTrieRemove(rootHash, adb.obsoleteDataTrieHashes)
	if err != nil {
		return err
	}
	adb.journalize(entry)
...
func (adb *AccountsDB) LoadAccount(address []byte) (AccountHandler, error) {
	...
	if len(address) == 0 {
		return nil, fmt.Errorf("%w in LoadAccount", common.ErrNilAddress)
	}
...
func (adb *AccountsDB) GetExistingAccount(address []byte) (AccountHandler, error) {
	...
	if len(address) == 0 {
		return nil, fmt.Errorf("%w in GetExistingAccount", common.ErrNilAddress)
	}
...
func (adb *AccountsDB) RevertToSnapshot(snapshot int) error {
	...
	if snapshot > len(adb.entries) || snapshot < 0 {
		return common.ErrSnapshotValueOutOfBounds
	}
...
func (adb *AccountsDB) Commit() ([]byte, error) {
	...
	adb.entries = make([]JournalAccountEntry, 0)
...
// SnapshotState triggers the snapshot process of the state trie
...
func (adb *AccountsDB) increaseNumCheckpoints() {
	...
	err := trieStorageManager.Database().Put(numAccountsCheckpointsKey, numCheckpointsVal)
	if err != nil {
