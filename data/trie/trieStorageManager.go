...
func getSnapshotsAndSnapshotId(snapshotDbCfg config.DBConfig) ([]data.SnapshotDbHandler, int, error) {
	...
	files, err := os.ReadDir(snapshotDbCfg.FilePath)
	if err != nil {
...
func (tsm *trieStorageManager) removeFromDb(rootHash []byte) error {
	...
	log.Debug("trie removeFromDb", "rootHash", rootHash, "len", len(hashes))
...
func (tsm *trieStorageManager) Close() error {
	...
	if errCheckpointsStorerClose != nil {
		log.Error("trieStorageManager.Close errMainStorerClose", "error", errCheckpointsStorerClose)
